...
void didCompressH264(void *outputCallbackRefCon,
    ...
    if (!CMSampleBufferDataIsReady(sampleBuffer)) {
        NSLog(@"didCompressH264 is not ready");
        return;
    }
...
-(void) start:(int)width height:(int)height {
    ...
    if (!initialized) {
        ...
        _error = @"H264: Not initialized";
        return;
    }
    
    __weak H264Encoder * weakSelf = self;
    dispatch_sync(aQueue, ^{
        ...
        if (status != 0 ) {
            ...
            weakSelf.error = @"H264: Unable to create a H264 session";
            return ;
        }
        ...
        VTSessionSetProperty(self->encodingSession, kVTCompressionPropertyKey_MaxKeyFrameInterval, 240);
        ...
        if (fd == -1)
        {
            ...
            weakSelf.error = @"H264: Unable to open the file";
        ...
        while (actualBytes > 0)
        {
            ...
            if (status != noErr)
            {
                ...
                weakSelf.error = @"H264: CMBlockBufferCreateWithMemoryBlock failed ";
            ...
            if (statusCode != noErr) {
                ...
                weakSelf.error = @"H264: CMSampleBufferCreate failed ";
            ...
            statusCode = VTCompressionSessionEncodeFrame(self->encodingSession,
                                                         imageBuffer,
            ...
            if (statusCode != noErr) {
                ...
                weakSelf.error = @"H264: VTCompressionSessionEncodeFrame failed ";
...
- (void) initEncode:(int)width  height:(int)height
{
    dispatch_sync(aQueue, ^{
        ...
        {
            NSLog(@"H264: Unable to create a H264 session");
            self->_error = @"H264: Unable to create a H264 session";
